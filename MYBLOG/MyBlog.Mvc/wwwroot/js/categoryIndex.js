$(document).ready(function () {
    $('#categoriesTable').DataTable({
        ...
        buttons: [
            {
                text: 'Ekle',
                attr: {   // placeholder attr özelliklerinden birisi ile eklenecek
                    id: "btnAdd",
                },
        ...
        language: {
            ...
            "editor": {
                ...
                "multi": {
                    ...
                    "noMulti": "Bu alan bir grup olarak değil ancak tekil olarak nebilir.",
    ...
    //End of datatable
    $(function () {
        const url = '/Admin/Category/Add/'; // category controller içindeki add actionunun adresi
        const placeHolderDiv = $('#modalPlaceHolder');
        $('#btnAdd').click(function () {
            $.get(url).done(function (data) { // data= categoryaddpartalview
                placeHolderDiv.html(data);
                placeHolderDiv.find(".modal").modal('show');
            });
        });
        //Ajax get / getting the ?CategoryAddPartial as Modal From extends here
        //Ajax POST | Posting the FormData as categoryAddDto starts from here
        placeHolderDiv.on('click', '#btnSave',
            function (event) {

                // bu div tetiklendiğinde çalışacak. btnSave idli elementine tıklandığında çalıtır. Neyi?-> fonksiyonu
                event.preventDefault();// butonun kendine tıklandığında sayfanın yenilenmemesini salar
                const form = $('#form-category-add');  // formu seç
                const actionUrl = form.attr('action'); // formdaki action url'i oku |
                const dataToSend = form.serialize();   // forun içinddeki veriyi categoryadddto ya çevirme işlemi
    ...
    <tr name="${categoryUpdateAjaxModel.CategoryDto.Category.Id}">
        ...
        <td>
             ...
             <button class="btn alert-danger btn-sm btn-delete" data-id="${categoryAddAjaxModel.CategoryDto.Category.Id}">
                 <span class="fas fa-minus-circle"></span>
             </button>
        </td>
    </tr>`;
    ...
    //Ajax POST | Posting the formdata as categoryadddto ens here
    // Ajax POST | Dleting a category starts from here
    //işleme başlamak için sil butonuna basılma olayını yakalamak gerekir.
    $(document).on('click', '.btn-delete', function (event) {
        const id = $(this).attr('data-id') //üzerine tıklanan buton
        ...
        Swal.fire({
            ...
            cancelButtonText: 'Hayır, Silmek İstemiyorum.!'
        }).then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    error: function (err) {
                        ...
                        Swal.fire(
                            "Başarısız",
                            `${categoryName} adlı kategori silinemedi.`,
                            "warning"
                        )
                    }
                })
    ...
    }); // Document-> mevcutta kulanılan sayfa

    $(function () {
        const url = '/Admin/Category/Update/';
        // üzerinden kategori seçeceğimiz, ekleyeceğimiz, güncelleyeceğimiz  tabloyu idsi ile seçtik
        const placeHolderDiv = $('#modalPlaceHolder');
        //btn-update tıklandığında fonksiyonu çalıştırdık
        $(document).on('click', '.btn-update', function (event) {
            event.preventDefault();
            const id = $(this).attr('data-id');
            //jQuery get işlemi. urş ve değişkenler verilir
            $.get(url, { categoryId: id }).done(function (data) {
                placeHolderDiv.html(data);
                //modal classına ait değerin bir modal olarak açılmasını belirttik. modal-> mevcut ekranda popup gibi açılan ekranlar.
                placeHolderDiv.find('.modal').modal('show');
            }).fail(function () {
                toastr.error("hata");
            });
        });
        //Ajax POST | Uptating Category starts here
            ...
            function (event) {
                ...
                $.post(actionUrl, dataToSend).done(function (data) {
                    ...
                    if (isValid) {
                        ...
                        // bir new tablerow oluşturduk fakat kullanabilkmek için bunu bşr jquery objesi haline getirmeliyiz
                        const newTableRowObject = $(newTableRow);
